...
// Default values
const DEFAULT_PLAYER_WS_PORT = 3333;
...
class PlayerServer {
    ...
    constructor(controller) {
        ...
        this.player_socket.on('connection', function connection(ws) {
            console.log('-> New connection of the player', player_socket);
	...
	//console.log(player_socket_clients_id)
        ...
        try {
			...
			//console.log('ENVOI')
                        ...
                        console.log(`Sending simulation output to player ${id_player}: ${JSON.stringify(json_output_player)}`);
        player_socket_clients[index].send(JSON.stringify(json_output_player));
    ...
    try {
        ...
        if (this.controller.model.getJsonSettings().verbose) 
            {console.log(`Sending ping to player ${id_player}: ${pingMessage}`);
        ws.send(pingMessage);}
        ...
        pongTimeout2Attempt[id_player] = setTimeout(() => {
            ...
            // Clear timeouts when the connection is closed or terminated
            clearTimeout(pongTimeout1Attempt[id_player]);
            clearTimeout(pongTimeout2Attempt[id_player]);
        }, 6000);
    }
    catch (error) {
        console.log("\x1b[31m-> Error when sending ping message\x1b[0m");
    ...
    notifyPlayerChange(id_player, json_player) {
        ...
        if (index != -1) {
            ...
            console.log(`Notifying player ${id_player} about change: ${JSON.stringify(concatenated_json_state_player)}`);
        player_socket_clients[index].send(JSON.stringify(concatenated_json_state_player));
