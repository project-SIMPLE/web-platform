...
class ConnectorGamaServer {
                ...
                if (typeof list_messages[index_messages] == "function") {
                    ...
                    if (this.controller.model.getJsonSettings().verbose) {
                        if (list_messages[index_messages]().expr != undefined) 
                            console.log("Expression or Command sent to Gama-platform Software: "+'\''+list_messages[index_messages]().expr+'\'' +"After sending the message, we should be Waiting for a response from the Gama platform server if there id no ...");
                        ...
                        if( list_messages[index_messages]().expr != undefined && (list_messages[index_messages]().expr.includes('create_player') || list_messages[index_messages]().expr.includes('remove_player'))) {
                            console.log("\x1b[1;34m-> Info : Command sent to Gama when player is a connected or removed: "+'\''+list_messages[index_messages]().expr+'\'' +" Waiting for the server to respond if the conditions are met");
                        }
                    }
                }
    ...
    connectGama() {
        ...
        gama_socket.onopen = function() {
            console.log("\x1b[1;32m-> Success : The application successfully connected to Gama Server");
        ...
        gama_socket.onmessage = function(event) {
            try {
                ...
                if (message.type == "SimulationStatus") {
                    if (controller.model.getJsonSettings().verbose) console.log("Message received from Gama Server: SimulationStatus");
                    console.log("-------------------Messages about experiment status received from Gama --------------------")
                    console.table(message);
                //    console.log(message);
                ...
                if (gama_error_messages.includes(message.type)) {
                    console.log("\x1b[31m->Error : Gama-platform error messages below:");
                    console.log("\x1b[37m---There is the error content we got from gama-platform : ",message);
        ...
        gama_socket.addEventListener('close', (event) => {
            ...
            if (event.wasClean) {
                ...
                console.log('\x1b[1;31m-> The connection with Gama Server interruped suddenly');
            }
        })
