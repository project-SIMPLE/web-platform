...
class App {
    ...
    constructor(controller) {
        ...
        this.app.get('/homepage', (req, res) => {
          res.sendFile('homepage.html', { root: 'views/homepage' });
        });
        ...
        this.app.get('/waiting', (req, res) => {
          res.sendFile('waiting.html', { root: 'views/waiting' });
        });
        ...
        this.server = this.app.listen(this.app_port, () => {
            console.log(`\x1b[1;34m-> Info : The application is listening on port : ${this.app_port}`+'. If you see this message it means that you have setted up correctly the application port')

        });

        this.server.on('error', (err) => {
          if (err.code === 'EADDRINUSE') {
            console.error(`\x1b[31m-> Error : The application port ${this.app_port} is already in use. Choose a different port in 'settings.json'.
            \x1b[33mConsider choose carefully the application port it will bee the one on which the entire app will be launched.\x1b[0m`);
          } 
          else {
